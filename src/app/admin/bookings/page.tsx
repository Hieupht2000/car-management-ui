 ...
 * Allows users to create, view, update, and cancel service appointments
 * Integrates with booking service for API operations
 */
"use client";
...
export default function BookingPage() {
    const { t, language, changeLanguage } = useTranslation('booking');
    ...
    const [showDealerEmailModal, setShowDealerEmailModal] = useState<boolean>(false);
    const [selectedBookingId, setSelectedBookingId] = useState<number | null>(null);
    const [dealerEmail, setDealerEmail] = useState<string>("");
    const [showAssignTechnicianModal, setShowAssignTechnicianModal] = useState<boolean>(false);
    const [selectedBookingForAssign, setSelectedBookingForAssign] = useState<BookingDTO | null>(null);
    const [technicians, setTechnicians] = useState<any[]>([]);
    const [selectedTechnicianId, setSelectedTechnicianId] = useState<number | null>(null);
    const [assigningTechnician, setAssigningTechnician] = useState<boolean>(false);
    ...
    const loadTechnicians = async () => {
        try {
            if (!token) {
                console.warn("No token available for loading technicians");
                return;
            }
            const data = await technicalService.getTechnicians(token);
            console.log("Loaded technicians:", data);
            console.log("Technician data structure:", data?.[0]);
            setTechnicians(data || []);
        } catch (error) {
    ...
    const handleAssignTechnician = async () => {
        ...
        try {
            ...
            await bookingService.assignTechnician(selectedBookingForAssign.booking_id!, selectedTechnicianId, token);

            // Refetch bookings to update UI
            const updatedBookings = await bookingService.getBookings(token);
    ...
    const HandelReminder = async (
        ...
        try {
            await bookingService.sendEmailReminder(bookingId, "reminderMessage", token);
            alert(`email send reminder#${bookingId}`);
        } catch (error: any) {
            console.error(error);
            alert(error.message || "send email reminder failed!");
        }
    };
                                    ...
                                    <div className="p-5 space-y-4">
                                        ...
                                        <div className="flex gap-2 text-sm">
                                             <span className="text-gray-600 font-medium">
                                                 Technician: <span className="text-gray-900">{booking.technicianName || technicians.find((t: any) => t.technicianId === booking.technicianId)?.fullName || "Not assigned"}</span>
                                             </span>
                                             <span className="text-gray-500">•</span>
                                             <span className="text-gray-600 font-medium">
                                                 Garage: <span className="text-gray-900">{booking.garageId}</span>
                                             </span>
                                         </div>
                                        ...
                                        <div className="flex gap-2 pt-3 border-t border-gray-100">
                                             {!booking.technicianId && (
                                                 <button
                                                     onClick={() => openAssignTechnicianModal(booking)}
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                        ...
                        <div className="mb-6">
                            <p className="text-sm text-gray-600 mb-4">
                                <strong>Booking:</strong> {selectedBookingForAssign.licensePlate} - {selectedBookingForAssign.fullName}
                            </p>

                            <label className="block text-sm font-semibold text-gray-700 mb-3">Select Technician</label>
                            <select
                            ...
                            >
                                ...
                                {technicians && technicians.length > 0 ? (
                                    technicians.map((tech: any, index: number) => (
                                        <option key={`tech-${tech.technicianId}-${index}`} value={tech.technicianId}>
                                            {tech.fullName || tech.name || "Unnamed"} (ID: {tech.technicianId || tech.id})
                                        </option>
                                    ))
                                ) : (
                ...
                <div className="p-6 space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <input type="text" name="licensePlate" value={form.licensePlate} onChange={handleChange} placeholder="License Plate *" className="px-4 py-3 border-2 text-gray-700 rounded-xl focus:border-blue-500 outline-none" />
                        </div>
                    </div>
                    ...
                    <div>
                         ...
                         <div className="space-y-4">
                             ...
                             {/* Time Slot Selector - Movie Ticket Style */}
